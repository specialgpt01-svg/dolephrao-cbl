...
/** คีย์ผู้ใช้คอลัมน์แรกของ Users / Logs — ใช้ชื่อผู้ใช้แทนเบอร์โทร (รองรับค่าเก่าที่เป็นตัวเลขใน Sheet) */
...
function getSources(actor) {
  try {
    ...
    const contentValues = SS.getSheetByName("Contents").getDataRange().getValues();
    ...
    const quizValues = SS.getSheetByName("Quizzes").getDataRange().getValues();
    ...
    const basesBySource = {};
    if (basesSheet && basesSheet.getLastRow() > 0) {
      ...
      if (values.length > 1) {
        ...
        for (let i = 1; i < values.length; i++) {
          const sid = String(values[i][idx.sourceId] || '').trim();
          if (!sid) continue;
          const isActive = idx.active < 0 ? true : toBool(values[i][idx.active]);
          if (!isActive) continue;
        ...
        Object.keys(basesBySource).forEach(function(sid) {
          basesBySource[sid].sort(function(a, b) {
  ...
  try {
    ...
    for (let i = 1; i < values.length; i++) {
      if (normalizeUsername(values[i][1]) === userStr) {
        results.push({
          timestamp: Utilities.formatDate(values[i][0], "GMT+7", "yyyy-MM-dd HH:mm:ss"),
          title: values[i][2],
...
function getUserProfileFullData(userId) {
  try {
    ...
    // 3. ดึงประวัติเกียรติบัตร
    const myHistory = getPassedHistory(userStr);
    
    // ส่งกลับรวดเดียวจบ
    return { status: "success", profile: myProfile, history: myHistory };
